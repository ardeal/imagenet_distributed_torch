...
def parse():
    ...
    parser.add_argument('-j', '--workers', default=16, type=int, metavar='N', help='number of data loading workers (default: 4) these are different from the processes that run the programe. they are just for data loading')
...
def accuracy(output, target, topk=(1,)):
    ...
    res = []
    for k in topk:
        # print(f'k=={k}, topk == {topk}, correct.size == {correct[:k]}')
        correct_k = correct[:k].contiguous().view(-1).float().sum(0, keepdim=True)
        # correct_k = correct[:k].reshape(...).float().sum(0, keepdim=True)
